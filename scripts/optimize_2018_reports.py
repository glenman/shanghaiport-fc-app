...
def normalize_team_name(name):
    ...
    if "Opponent" in name:
        return name
    if "Shanghai" in name and "SIPG" in name:
        return "上海海港"
    return name
...
def normalize_competition_name(name):
    """标准化赛事名称"""
    name = name.strip()
    if "Chinese Football Association Super League" in name:
        return "中国足球协会超级联赛"
...
def is_shanghai_port_team(team_data):
    ...
    # 检查队名或是否有特定球员
    if "SIPG" in name or "Shanghai" in name and "Port" in name or name == "上海上港" or name == "上海海港":
        return True
    # 检查formation中是否有SIPG
    ...
    for player in lineup:
        ...
        if "Yan Junling" in p_name or "Hulk" in p_name or "Oscar" in p_name:
            return True
    return False
...
def process_file(json_path, player_map, schedule):
    ...
    try:
        ...
        if round_num and round_num in schedule:
            ...
            correct_home_is_port = sched['home'] == "上海海港" or sched['home'] == "上海上港"
        ...
        if should_swap:
            ...
            data['teams']['home'] = away_team
            data['teams']['away'] = home_team
            ...
            for team_key in ['home', 'away']:
                ...
                for sub in team.get('substitutions', []):
                    if sub.get('team') == "home":
                        sub['team'] = "away"
                    elif sub.get('team') == "away":
                        sub['team'] = "home"
        ...
        for team_key in ['home', 'away']:
            ...
            if team_key == 'home' and is_home_port:
                should_process = True
            if team_key == 'away' and is_away_port:
                should_process = True
        ...
        if 'competition' in data['match_info']:
            ...
            if 'season' in comp:
                comp['season'] = SEASON  # 修正赛季
            if 'round' in comp:
                comp['round'] = f"第{round_num}轮"
        
        # 汉化场地名称
        if 'venue' in data['match_info']:
...
def main():
    print("=" * 60)
    print(f"2018赛季赛事报告优化处理")
    print("=" * 60)
    
    # 加载数据
    player_map = load_roster(ROSTER_CSV)
