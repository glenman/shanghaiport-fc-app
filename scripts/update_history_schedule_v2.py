...
def load_coach_data():
    ...
    for sheet_name in xls.sheet_names:
        ...
        try:
            ...
            # Extract coach name from sheet (this is Shanghai Port's coach)
            sp_coach = sheet_name
            ...
            for idx, row in df.iterrows():
                ...
                if home_team in ['上海海港', '上海上港', '上海东亚']:
                    ...
                    # Neither team is Shanghai Port, skip
                    continue
...
def match_json_to_excel(json_match, excel_matches):
    ...
    for excel_match in excel_matches:
        if excel_match['date'] == json_date:
            # Check if home/away teams match
...
def main():
    ...
    for match in history_data:
        # Remove old 'coach' field if it exists
        if 'coach' in match:
            del match['coach']
            old_coach_removed += 1
