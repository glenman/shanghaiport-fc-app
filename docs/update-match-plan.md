...
4. **区分球队类型** - 分别更新一线队和海港B队的数据文件

## 海港B队更新要点

1. **更新文件**：`data/schedule_b.json`
2. **赛事报告链接**：使用通用模板 `match-report.html`
3. **URL参数格式**：
   - 第2轮：`?date=2026-04-05&type=中乙&round=2`
   - 第3轮：`?date=2026-04-10&type=中乙&round=3`
4. **比赛数据文件**：`data/{日期}-{赛事类型}-{轮次}.json`（如：`data/2026-04-05-中乙-第2轮.json`、`data/2026-04-10-中乙-第3轮.json`）
...
```json
{
  "id": 3.2,                    // 比赛ID + 小数（如3.2表示第3轮第2个报告）
  "matchId": 3,                  // 对应的比赛ID（与schedule_b.json对应）
  "round": "第3轮",             // 轮次
  "date": "2026-04-10",         // 比赛日期
  "homeTeam": "上海海港B队",        // 主队
  "awayTeam": "武汉江城",          // 客队
  "result": "0-0",              // 比分（0:0战平）
  "reportType": "中乙",          // 赛事类型
  "reportTitle": "上海海港B队0-0武汉江城 赛后报告"  // 报告标题
}
```
